...
func TestOutputJSONWithWarnings_NoCommentsSubtaskError(t *testing.T) {
	...
	err := outputJSONWithWarnings(nil, warnings, 1, 10, 5, 15, 0, 0, time.Second, "", nil, nil)
...
func TestOutputJSONWithWarnings(t *testing.T) {
	...
	err := outputJSONWithWarnings(comments, warnings, 5, 100, 50, 150, 10, 5, 3*time.Second, "summary", map[string]int64{"file_read": 3}, nil)
